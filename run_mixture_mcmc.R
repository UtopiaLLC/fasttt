...
run_mixture_mcmc <- function(stops, output_filename, iter = 5000, warmup = NULL, chains = 1, adapt_delta = 0.95, max_treedepth = 12, sample_from_prior = FALSE, verbose = FALSE, simulation=FALSE, model_file = 'mixture_model.stan') {
  # checked. 
  if (is.null(warmup)) {
...
run_threshold_test <- function(file_prefix, model_file){
  ...
  message(sprintf('Running threshold test on %s: %i locations, %i races, saving results to %s', 
                  ...
                  out_name))
  output = run_mixture_mcmc(stops, out_name, iter=5000, chains=1, adapt_delta=.9, max_treedepth=12, model_file=model_file)
}
